...
function searchInvoices($conn, $searchTerm) {
    $results = [];
    $stmt = $conn->prepare("SELECT * FROM invoices WHERE client_name LIKE ? OR invoice_number LIKE ?");
...
// Function to delete invoice from the database and file system
function deleteInvoice($conn, $invoiceNumber, $clientName) {
    // Delete the PDF file if it exists
    $stmt = $conn->prepare("SELECT pdf_file FROM invoices WHERE invoice_number = ? AND client_name = ?");
    $stmt->bind_param("ss", $invoiceNumber, $clientName);
    $stmt->execute();
    $result = $stmt->get_result();
    
    if ($invoice = $result->fetch_assoc()) {
        $pdfPath = $invoice['pdf_file'];
    ...
    $stmt = $conn->prepare("DELETE FROM invoices WHERE invoice_number = ? AND client_name = ?");
    $stmt->bind_param("ss", $invoiceNumber, $clientName);
    $stmt->execute();
    $stmt->close();
}
...
if (isset($_POST['delete'])) {
    ...
    deleteInvoice($conn, $invoiceNumber, $clientName);
    header("Location: /search_json?search=" . $_POST['searchTerm']);
    exit();
}
...
if (isset($_GET['search'])) {
    ...
    $searchResults = searchInvoices($conn, $searchTerm);
}
?>
